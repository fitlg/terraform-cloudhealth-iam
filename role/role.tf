...
policy = <<POLICY
{
  ...
  "Statement": [
    ...
    ${var.s3-billing-bucket == "" ? "" : format(
",%s",
data.template_file.default-s3-billing-bucket-policy.rendered,
)}
    ${var.s3-cloudtrail-bucket == "" ? "" : format(
",%s",
data.template_file.default-s3-cloudtrail-bucket-policy.rendered,
)}
    ${var.s3-cur-bucket == "" ? "" : format(
",%s",
data.template_file.default-s3-cur-bucket-policy.rendered,
)}
    ${var.s3-config-bucket == "" ? "" : format(
",%s",
data.template_file.default-s3-config-bucket-policy.rendered,
)}
    ${var.s3-ecs-bucket == "" ? "" : format(
",%s",
data.template_file.default-s3-ecs-bucket-policy.rendered,
)}
